    ...
    for (uint32_t i = 0; i < agg_exprs_.size(); i++) {
      switch (agg_types_[i]) {
        ...
        case AggregationType::CountAggregate:
          if (!input.aggregates_[i].IsNull()) {
            if (result->aggregates_[i].IsNull()) {
              result->aggregates_[i] = {INTEGER, 0};
            }
            result->aggregates_[i] = result->aggregates_[i].Add({INTEGER, 1});
          }
          break;
        case AggregationType::SumAggregate:
          if (!input.aggregates_[i].IsNull()) {
            if (result->aggregates_[i].IsNull()) {
              result->aggregates_[i] = {input.aggregates_[i].GetTypeId(), 0};
            }
        ...
        case AggregationType::MaxAggregate:
          if (!input.aggregates_[i].IsNull()) {
            if (result->aggregates_[i].IsNull() ||
                result->aggregates_[i].CompareLessThanEquals(input.aggregates_[i]) == CmpBool::CmpTrue) {
              result->aggregates_[i] = input.aggregates_[i];
            }
          }
          break;
