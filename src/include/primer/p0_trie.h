...
class TrieNode {
 public:
  ...
  TrieNode(TrieNode &&other_trie_node) noexcept {
    this->SetEndNode(other_trie_node.is_end_);
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    if (key_char != child->key_char_ || child == nullptr || this->HasChild(key_char)) {
      return nullptr;
    }
    auto iter = this->children_.emplace(key_char, std::move(child));
...
class Trie {
 ...
 public:
   ...
   * character.
   */
  Trie() { root_ = std::make_unique<TrieNode>(default_char_); }
  ...
  bool Insert(const std::string &key, T value) {
    ...
    latch_.WLock();
    TrieNode *p = &*root_;
    int last_idx = key.size() - 1;
    int idx = 0;
    char ch;
    // 把 key[0, lastIdx) 加到 root 下
    while (idx < last_idx) {
      ch = key[idx++];
      if (!p->HasChild(ch)) {
        p = &**(p->InsertChildNode(ch, std::make_unique<TrieNode>(ch)));
      } else {
        p = &**(p->GetChildNode(ch));
      }
    }
    ch = key[idx];
    auto child = p->GetChildNode(ch);
    if (child != nullptr) {
      if ((*child)->IsEndNode()) {
        latch_.WUnlock();
        return false;
      }
      // 如果 key[lastIdx] 不是终端节点，将其变为终端节点
      child->reset(new TrieNodeWithValue<T>(std::move(**child), value));
    } else {
      p->InsertChildNode(ch, std::make_unique<TrieNodeWithValue<T>>(ch, value));
    }
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    auto pre = &root_;
    for (char ch : key) {
      auto node = pre->get()->GetChildNode(ch);
      if (node == nullptr) {
        latch_.WUnlock();
        return false;
      }
      v.emplace_back(std::make_tuple(ch, pre));
      pre = node;
    }
    while (!v.empty()) {
      auto tmp = v.back();
      v.pop_back();
      char ch = std::get<0>(tmp);
      auto pre = std::get<1>(tmp);
      auto child = pre->get()->GetChildNode(ch);
      // 如果被删除的节点有附属节点，则不删除
      if (child != nullptr && child->get()->HasChildren()) {
        break;
      }
      pre->get()->RemoveChildNode(ch);
    }
    latch_.WUnlock();
    return true;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    auto node = dynamic_cast<TrieNodeWithValue<T> *>(p);
    if (node == nullptr) {
      latch_.RUnlock();
      return {};
    }
